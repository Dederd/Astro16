        ...
        <div class="card form-section">
          ...
          <div class="courier-grid">
            <div
              ...
              @click="form.courier_service = courier.value"
            >
      ...
      <div class="order-summary">
        <div class="summary-card card">
          ...
          <div class="summary-row">
            <span>Ukuran</span>
            <span class="summary-val">{{ sizeLabel }}</span>
          </div>
          <div class="summary-row">
            <span>Jumlah Tangkai</span>
            <span class="summary-val">{{ stemCount }} tangkai</span>
          </div>

          <div class="summary-flowers-section">
          ...
          <div class="summary-row">
            <span>Kurir</span>
            <span class="summary-val">{{ form.courier_service ? form.courier_service.toUpperCase() : '—' }}</span>
          </div>
...
const couriers = [
  { value: 'jne', name: 'JNE', logo: '📦', desc: 'Reguler 2-3 hari' },
  { value: 'jnt', name: 'J&T Express', logo: '🚚', desc: 'Reguler 1-2 hari' },
  { value: 'sicepat', name: 'SiCepat', logo: '⚡', desc: 'Same day tersedia' },
]

const paymentLoading = ref(false)
const error = ref('')

// Design info (support catalog & AI mode)
...
const sizeLabel = computed(() => {
  if (store.orderMode === 'catalog') return 'Katalog'
  return store.selectedSize === 'small'
    ? store.selectedDesign?.small?.label
    : store.selectedDesign?.large?.label
})
...
async function handlePayment() {
  ...
  try {
    ...
    // BUG FIX #4: log payload untuk debug
    ...
    if (window.snap && token) {
      ...
      error.value = 'Gagal mendapat token pembayaran. Pastikan Midtrans dikonfigurasi dengan benar.'
      paymentLoading.value = false
    }
  } catch (e) {
    // BUG FIX #4: tampilkan error detail dari backend
...
function gradientFor(style) {
  const g = {
    ...
    Modern: 'linear-gradient(135deg, #D4E8E0, #8ABFAD)',
    Natural: 'linear-gradient(135deg, #E8F5E8, #8BC888)',
    Classic: 'linear-gradient(135deg, #FFF8F0, #F5D5B0)',
...
.preview-visual { width: 64px; height: 64px; border-radius: 12px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
...
.summary-row { display: flex; justify-content: space-between; padding: 8px 0; font-size: 0.88rem; color: var(--warm-gray); border-bottom: 1px solid var(--light-gray); }
.summary-val { font-weight: 500; color: var(--charcoal); }

.summary-flowers-section { padding: 12px 0; }
.summary-flowers-label { display: block; font-size: 0.75rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.08em; color: var(--warm-gray); margin-bottom: 8px; }
.summary-flowers-list { display: flex; flex-direction: column; gap: 4px; }
.summary-flower-row { display: flex; justify-content: space-between; font-size: 0.85rem; color: var(--charcoal); }
.summary-divider { height: 1px; background: var(--light-gray); margin: 12px 0; }
.summary-total { display: flex; justify-content: space-between; align-items: center; font-weight: 500; }
.total-price { font-family: var(--font-display); font-size: 1.4rem; color: var(--deep-rose); }

.error-msg { background: #FFEBEE; color: #C62828; padding: 12px 16px; border-radius: var(--radius-sm); font-size: 0.85rem; margin-bottom: 12px; border: 1px solid #FFCDD2; }

.pay-btn { width: 100%; justify-content: center; padding: 16px; font-size: 1rem; margin-bottom: 12px; }
.payment-note { font-size: 0.78rem; color: var(--warm-gray); text-align: center; line-height: 1.6; margin-bottom: 16px; }
